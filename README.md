...
- подробная страница вуза: кампусы и общежития, инфраструктура, климат,
  транспорт до соседних городов, ориентиры города, стоимость жизни, сроки
  подачи, скидки вуза, грант провинции и государственный грант;
- сравнение до четырёх вузов в одной таблице;
- отдельная страница о государственном гранте Китая (CSC).

## Откуда берутся данные страницы вуза

Часть полей карточки вуз не публикует в машиночитаемом виде, поэтому каталог
различает два источника.

**Справочник `campus-profile.mjs`** собирает предварительную оценку по городу и
уровню вуза: климатические нормы, расстояния до соседних городов, ориентиры
города, типовые условия в общежитии, набор инфраструктуры, стоимость жизни,
языки обучения, сроки приёма, скидки вуза и грант провинции. Такие значения
показываются с пометкой «предварительно», а запись хранится со статусом
`requires_verification`, пока редактор не сверит её с официальной страницей.

**Ручная проверка через CSV-импорт** заполняет то, что вывести нельзя: год
основания, площадь территории, список кампусов с датами реновации и состоянием
общежитий, число студентов и иностранных студентов. Пока поле пустое, карточка
показывает «уточняется». Значения, введённые редактором, переживают повторный
`npm run db:seed` — сид только дозаполняет пустые поля.

Оценка каталога (балл из десяти) считается в `catalogueRating` из места в
рейтинге Китая и мира, доступности государственной стипендии и безопасности
города. Отзывы попадают на карточку только с флагом `verified`, который
...
| `province` | Нет | Провинция вуза, например `Цзянсу` |
| `founded_year`, `campus_area_ha` | Нет | Год основания и площадь территории в гектарах |
| `students_count`, `international_students` | Нет | Число студентов всего и иностранных |
| `campuses` | Нет | JSON-массив кампусов (см. ниже) |

Колонка `campuses` принимает JSON-массив. Пустая ячейка ничего не затирает:

```json
[{"name": {"ru": "Главный кампус", "en": "Main campus"},
  "founded_year": 1911, "renovated_year": 2019,
  "area_ha": 395, "dorm_condition": "good"}]
```

`dorm_condition` принимает `good`, `fair` или `poor`.
